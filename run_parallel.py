...
class ReportGenerator:
    ...
    def __init__(self, start_date , end_date , period, interval):
        ...
        try:
            ...
            # store the imported OpenAI class on the instance so other methods can use it
            self.OpenAI = OpenAI
    ...
    def get_deepseek_analysis(self, ticker, model="deepseek-r1-250120"):
        ...
        # 检查信心指数是否大于75，信心指数格式："信心指数：XX"
        if "信心指数" in formatted_output:
            try:
                index_start = formatted_output.index("信心指数")
                index_value_start = formatted_output.index("：", index_start) + 1
                #长度为2
                index_value_end = 2 + index_value_start
                confidence_index_str = formatted_output[index_value_start:index_value_end].strip().replace("%", "").replace("*", "")
                confidence_index = float(confidence_index_str)
